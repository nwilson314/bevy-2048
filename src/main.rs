...
use rand::prelude::*;
use std::{cmp::Ordering, collections::HashMap, convert::{TryFrom, TryInto}, ops::Range};
...
fn render_tile_points(
    ...
    for (points, children) in tiles.iter() {
        if let Some(entity) = children.first() {
            ...
            let mut text_selection = text.sections.first_mut()
                .expect("expect first section to be accessible as mutable");
            text_selection.value = points.value.to_string();
            if text_selection.value.parse::<i32>().unwrap() > 100 {
                text_selection.style.font_size = 30.0;
            }
        }
    }
}
...
fn board_shift(
    ...
    if let Some(board_shift) = shift_direction {
        let mut it = tiles
            .iter_mut().sorted_by(|a, b| board_shift.sort(&a.1, &b.1))
            .peekable();
